...
class SpellAnimationPanel(rompanel.ROMPanel):
    ...
    def init(self):
        ...
        #self.rom.data["spell_animations"][0*3].hexlify()
        
        #inst = wx.StaticText(self, -1, "Edit spellAnim graphics.")
        #inst.Wrap(inst.GetClientSize()[0])
        
        leftSizer = wx.BoxSizer(wx.VERTICAL)
        
        #sbs1 = wx.StaticBoxSizer(wx.StaticBox(self, -1, "1. Select a spellAnim."), wx.VERTICAL)
        #sbs2 = wx.StaticBoxSizer(wx.StaticBox(self, -1, "Chars/scenes using this spellAnim"), wx.VERTICAL)
        ...
        """self.spellAnimList = wx.ComboBox(self, wx.ID_ANY, size=(200,-1))
        self.spellAnimList.AppendItems([bs.name for bs in self.rom.data["spell_animations"]])
        self.spellAnimList.SetSelection(0)

        sbs1.Add(self.spellAnimList, 0, wx.ALL, 10)
        sbs1.Add((0,10))"""
        ...
        self.editPanel = rompanel.SpritePanel(self, wx.ID_ANY, 16*8, 16*8, self.palette, scale=3, bg=16)
        ...
        #self.animPanel.refreshSprite(self.rom.data["spell_animations"][0].pixels
        ...
        #wx.EVT_COMBOBOX(self, self.spellAnimList.GetId(), self.OnSelectSpellAnim)
    ...
    def printrb(self, evt):
        ...
        #print self.spellAnim.raw_bytes
    ...
    def refreshPixels(self):
        ...
        #f.write(self.spellAnim.raw_bytes)
        #f.close()
        ...
        #self.charList.Clear()
        #self.charList.Append(hex(self.rom.tables["spell_animations"][int(self.spellAnim.name.split(" ")[1])*3 + self.side]))
        #self.charList.Append(`len(self.spellAnim.raw_bytes)`)
        #self.charList.Append(`len(h)`)
        
        #h = h.replace("\n","").replace(" ","").replace(":","")
        
        #f = open("%s %i.txt" % (self.spellAnim.name, self.side), "wb")
        #f.write(binascii.unhexlify(h))
        #f.close()
        
        #if self.frame == 0:
        #    self.testPanel.refreshSprite(self.rom.spellAnimSubroutine(h).pixels
        #else:
        #    self.testPanel.refreshSprite(self.rom.spellAnimSubroutine(h).pixels2
        #self.testPanel.Refresh()
    ...
    def OnSelectSpellAnim(self, evt):
        self.changeSpellAnim(evt.GetSelection())
    ...
    def changeSpellAnim(self, num=None):
        ...
        if num is not None:
            ...
            #self.frameList.AppendItems(["Frame %i" % i for i,p in enumerate(self.spellAnim.frames)])
            #self.frameList.SetSelection(self.curFrameIdx)
            
            #self.paletteList.Clear()
            #self.paletteList.AppendItems(["Palette %i" % i for i,p in enumerate(self.spellAnim.palettes)])
