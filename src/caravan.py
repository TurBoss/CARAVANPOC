...
import asm, settings
import rom, panellist, rompanel, changelog, window, consts, layouttree, util, temp, layout
...
class MainFrame(window.CaravanParentFrame):
    ...
    def __init__(self, parent, id, app):
        ...
        newFileMenu.Append(addID("SAVE_PROJECT_AS"), "&Save Project As...\tCtrl-Shift-S"," Save the currently loaded project under a different filename.")
        ...
        fileMenu.Append(addID("SAVEAS"), "&Save ROM As...\tCtrl-Shift-S"," Save the currently loaded Shining Force II ROM under a different filename (.BIN only)")
        fileMenu.AppendSeparator()
    ...
    def showError(self, text):
        ...
        if not self.isErring:
            ...
            dlg = wx.MessageDialog(self, text + "\nCopy the error to the clipboard?", self.baseTitle, wx.YES_NO | wx.ICON_ERROR)
            error = dlg.ShowModal()
            ...
            if error == wx.ID_YES:
                ...
                if res:
                    ...
                    wx.MessageDialog(self, "Copy failed.", self.baseTitle + " -- Error", wx.OK | wx.ICON_ERROR).ShowModal()
    ...
    def OnClose(self, evt):
        ...
        if success == wx.ID_NO:
            ...
            if self.rom.dataFile:
                ...
                dlg = wx.MessageDialog(self, "Close external data file '%s' as well?" % pathjoin(self.datadirname,self.datafilename),
    ...
    def initLayoutTree(self):
        ...
        pane = aui.AddPane(tree, wx.aui.AuiPaneInfo().Caption('Layout Tree').CloseButton(False).MinSize((130,-1)).Floatable(False))
    ...
    def saveFile(self, path=""):
        ...
        if not self.pendingCancel:
            ...
            dlgSavingText.SetLabel("%i of %i section%s saved properly." % (len(sectionsToUpdate), len(sectionsToUpdate), ["","s"][len(sectionsToUpdate) > 1]))
    ...
    def createInitialSubPanel(self):
        ...
        text = wx.StaticText(panel, -1, "Double-click an entry in the tree to the left to edit that section of Shining Force II.")
    ...
    def resetHelpText(self):
        ...
        self.helpText.SetLabel("Move the mouse over an item in the interface for a detailed message describing its function.")
...
class NewProjectDialog(wx.Dialog):
    ...
    def __init__(self, parent):
        ...
        self.romCtrl = romCtrl = fbb.FileBrowseButtonWithHistory(self, wx.ID_ANY, size=(400,-1), labelText="ROM File: ", dialogTitle="Select a ROM File (.bin)", fileMask="*.bin")
    ...
    def OnAddGameSpec(self, evt):
        ...
        dlg = wx.FileDialog(self, "Select a Caravan Game Specification File (.caravan-game)", "", "", "*.caravan-game", wx.OPEN)
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                try:
                    ...
                    if self.parent.settings.games.has_key(obj.name):
                        res = wx.MessageDialog(self, "A Game Specification with the name '%s' is already loaded.  Change its file to %s?" % (obj.name, path), self.parent.baseTitle, wx.YES | wx.NO | wx.ICON_QUESTION).ShowModal()
                    ...
                    if res == wx.ID_YES:
                        ...
                        wx.MessageDialog(self, "Added '%s'." % obj.name, self.parent.baseTitle, wx.OK | wx.ICON_INFORMATION).ShowModal()
                    
                    break
                    
                except EOFError, e:
                    wx.MessageDialog(self, "%s is not a valid Caravan Game Specification File." % path, self.parent.baseTitle + " -- Error", wx.OK | wx.ICON_ERROR).ShowModal()
    ...
    def OnRemoveGameSpec(self, evt):
        ...
        res = wx.MessageDialog(self, "Are you sure you want to remove '%s'?" % name, self.parent.baseTitle, wx.YES | wx.NO | wx.ICON_QUESTION).ShowModal()
        ...
        if res == wx.ID_YES:
            ...
            wx.MessageDialog(self, "Removed '%s'." % name, self.parent.baseTitle, wx.OK | wx.ICON_INFORMATION).ShowModal()
    ...
    def OnConfirm(self, evt):
        ...
        try:
            ...
            wx.MessageDialog(self, "Invalid ROM file.", self.parent.baseTitle + " -- Error", wx.OK | wx.ICON_ERROR).ShowModal()
            return
...
#mw = AppFrame(app, None, -1, u"Hello")
mw.Show(True)
app.SetTopWindow(mw)
# OLD window.mapViewer = window.MapViewer(mw, -1, mw)
